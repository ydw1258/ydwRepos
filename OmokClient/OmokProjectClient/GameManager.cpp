...
void GameManager::Draw(HDC hdc)
{
	...
	case LOBBY:
	{
		...
		buttonFont.Draw("∞‘¿” ¡æ∑·", 20, 780, 570, "Resources/oldgameFont.ttf", RGB(255, 0, 0));
	...
	case INGAME:
	{
		...
		char buf[128];
		sprintf(buf, "%d π¯πÊ %dπ¯¬∞ ¿Ø¿˙", roomIndex, userIndexInRoom);
		...
		gamestartFont.Draw("∞‘¿” Ω√¿€", 20, 350, 730, "Resources/oldgameFont.ttf", RGB(0, 0, 0));
...
bool GameManager::ProcessPacket(char * szBuf, USER_INFO_STRING& userinfo, int len, WPARAM wParam)
{
	...
	{
	case PACKET_INDEX_FIRST_CONNECT://√π ø¨∞·(≈¨∂Û¿Ãæ∆Æ Ω««‡)
	{
	...
	break;
	case PACKET_INDEX_LOGIN_RET://∑Œ±◊¿Œ Ω√µµ
	{
		PACKET_TRY_LOGIN packet;
		memcpy(&packet, szBuf, header.wLen);
		...
		{
			SetWindowText(LOGINInput[0], "∑Œ±◊¿Œ Ω«∆–");
		}
	}
	break;
...
void GameManager::DrawCurUsers(HDC hdc)
{
	...
	case LOGIN:
		break;
	case LOBBY:
		buttonFont.Draw("∑Œ∫Ò «√∑π¿ÃæÓ ∏Ò∑œ", 20, 710, 130, "Resources/oldgameFont.ttf", RGB(0, 0, 0));
		...
		break;
	case INGAME:
		buttonFont.Draw("¿Œ∞‘¿” «√∑π¿ÃæÓ ∏Ò∑œ", 20, 730, 200, "Resources/oldgameFont.ttf", RGB(0, 0, 0));
		if (isGameStart)
		{
			buttonFont.Draw("∞‘¿” ¡¯«‡ ¡ﬂ", 20, 730, 150, "Resources/oldgameFont.ttf", RGB(0, 0, 0));
		}
		else
		{
			buttonFont.Draw("¥Î±‚ ¡ﬂ", 20, 730, 150, "Resources/oldgameFont.ttf", RGB(0, 0, 0));
		}
...
void GameManager::DrawRooms(HDC hdc)
{
	...
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 2; j++)
		{
			...
			sprintf(buf, "%d π¯ πÊ ¿Áπ’¥¬ ø¿∏Ò %d / %d", i * 2 + j, mapRoomPlayers[i * 2 + j], 4);
