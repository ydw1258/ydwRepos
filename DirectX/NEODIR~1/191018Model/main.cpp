...
HRESULT  InitGeometry()
{
	...
	if (FAILED(D3DXLoadMeshFromXA("Tiger.x", D3DXMESH_SYSTEMMEM, g_pD3DDevice, NULL, &pD3dXMtrlBuffer, NULL, &g_dwNumMaterials, &g_pMesh)))
	{
		MessageBox(NULL, "Couldnot fid tiger.x", "ERROR", MB_OK);
	}
	...
	for (int i = 0; i < g_dwNumMaterials; i++)
	{
		...
		if (d3dxMaterials[i].pTextureFilename != NULL && strlen(d3dxMaterials[i].pTextureFilename) > 0)
		{
			...
			{
				MessageBox(NULL, "Could not find texture map", "Texture", MB_OK);
				return E_FAIL;
			}
		}
	}
